...
/**
 * 说明：LinearLayoutManager竖向滑动可行
 * Created by jjs on 2018/8/21.
 */

public class MoveLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void fillViews(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (getChildCount() == 0) {
            ...
            for (int i = getChildCount() - 1; i >= 0; i--) {
                ...
                //item底部超出recy上边界时，删除
                if (mLayoutStatus.orientation == LayoutState.OrientationStart && getDecoratedBottom(child) + getParams(child).bottomMargin < 0) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (mLayoutStatus.orientation == LayoutState.OrientationEnd && getDecoratedTop(child) - getParams(child).topMargin + mLayoutStatus.dt > 0 && position > 0 && i == 0) {
